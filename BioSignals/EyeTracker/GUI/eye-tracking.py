...
def process_dual_cameras():
    ...
    mode_open = False
    current_label = None
    waiting_for_next_toggle = False  # helps debounce '1' trigger


    while True:
        hex_x = "00"  # fallback definition to prevent UnboundLocalError
        ...
        # === Trigger 'r' key behavior from serial data ===
        r_triggered = False
        ...
        if ser.in_waiting > 0:
            try:
                ...
                if incoming:  # any non-empty input triggers it
                    r_triggered = True
            except:
                pass

        # === TOGGLE MODE SELECTION with 'r' key ===
        if (key == ord('r') or r_triggered) and calibration_complete:

            if mode_open:
                # === CLOSE current mode ===
                print(f"[INFO] Mode closed via 'r': {current_label}")
                try:
                    cv2.destroyWindow("Selected Mode")
                except:
                    print("[WARNING] Could not destroy label window")
                current_label = None
                mode_open = False

            elif smoothed_pupil_coords != (0, 0):
                print("[INFO] 'r' pressed — checking gaze for label selection")
                ...
                label = label_map.get(closest_point, None)
                if label is not None:
                    print(f"[INFO] Selected mode based on gaze: {label}")
                    current_label = label
                    mode_open = True

                    try:
                        mode_screen = np.ones((HEIGHT, WIDTH, 3), dtype=np.uint8) * 255
                        cv2.namedWindow("Selected Mode", cv2.WINDOW_NORMAL)
                        cv2.setWindowProperty("Selected Mode", cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
                        cv2.putText(mode_screen, label, (WIDTH // 4, HEIGHT // 2), FONT, 3, (0, 0, 0), 5, cv2.LINE_AA)
                        cv2.imshow("Selected Mode", mode_screen)
                        cv2.waitKey(2000)  # Show label window for 2 seconds
                        cv2.destroyWindow("Selected Mode")
                    except Exception as e:
                        print(f"[ERROR] Failed to show label window: {e}")
                else:
                    print("[WARNING] Could not determine label from gaze")
        ...
        # Compute hex_x from pupil deviation before serial write
        if calibration_complete and reference_point is not None and smoothed_pupil_coords != (0, 0):
            ...
            hex_x = "00"
# === Always send serial after calibration ===
        if calibration_complete:
            if mode_open and current_label:
...
# ---------------------------------------------------------------------------
def launch_gui():
    # Launch the GUI from 2.2 GUI.py in a subprocess
    subprocess.Popen(['python', '2.2 GUI.py'])

if __name__ == "__main__":
    # Launch the GUI in a separate thread so it runs concurrently
    gui_thread = threading.Thread(target=launch_gui)
    gui_thread.start()

    # Start pupil tracking as usual
